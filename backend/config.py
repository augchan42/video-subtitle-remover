...
# Assemble split model files if needed
try:
    from fsplit.filesplit import Filesplit
    for _dir in [LAMA_MODEL_PATH, DET_MODEL_PATH, VIDEO_INPAINT_MODEL_PATH]:
        manifest = os.path.join(_dir, 'fs_manifest.csv')
        if os.path.exists(manifest):
            # Check if already assembled by looking for known output files
            assembled = any(
                f for f in os.listdir(_dir)
                if not f.startswith('fs_') and not '_' in f.split('.')[0]
                and f.endswith(('.pt', '.pth', '.pdiparams'))
            )
            if not assembled:
                fs = Filesplit()
                fs.merge(input_dir=_dir)
except ImportError:
    pass  # fsplit not installed, assume models already assembled

# Use system ffmpeg if bundled one not available
import shutil
...
_bundled = os.path.join(BASE_DIR, 'ffmpeg', ffmpeg_bin)
if os.path.exists(_bundled):
    FFMPEG_PATH = _bundled
    try:
